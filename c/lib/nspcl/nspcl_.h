...
/*
//
// Class - Connector.  Base behaviour for emitters/receptors.
//
...
class Connector :
	...
	{
	public :
	Connector ( Emitter * );							// Constructor
	Connector ( Receptor * );							// Constructor
	virtual ~Connector ( void );						// Destructor

	// Run-time data
	Emitter		*pEmt;									// Parent emitter
	Receptor		*pRcp;									// Parent receptor
	IList			*pDefConn;								// Defined connections for connector
	INamespace	*pSpc;									// Reference to namespace
	IDictionary	*pParN;									// Reference to parent node
	IDictionary	*pParG;									// Reference to parent graph
	adtString	strName;									// Connector name
	adtValue		vLast;									// Last value processed
	IReceiver	*pRcvr;									// Receiver
	ConnList		*pConns;									// Run-time connectors
	...
	// Internal utilties
//	STDMETHOD(connect)	( bool );
//	STDMETHOD(connect)	( const WCHAR *, bool );
	};

//
// Class - ConnIt.  Iterator for a connector.
//
...
class ConnIt :
	...
	{
	public :
	ConnIt ( Emitter *, bool ); 						// Constructor
	ConnIt ( Receptor *, bool ); 						// Constructor

	// Run-time data
	Emitter	*pEmt;										// Parent emitter
	Receptor	*pRcp;										// Parent receptor
	bool		bKeys;										// Iterating keys/values
	int		iIdx;											// Enumeration index
...
//
// Class - Emitter.  Default emitter class.
//

class Emitter :
	public CCLObject,										// Base class
	public Connector,										// Base class
	public IEmitter										// Interface
	{
	public :
	Emitter ( void );										// Constructor

	// Run-time data

	// 'IEmitter' members
	STDMETHOD(connect)	( IReceptor *, bool );
	STDMETHOD(connected)	( void );
	STDMETHOD(emit)		( const ADTVALUE & );
...
//
// Class - EmitterLoc.  Base class for a location emitter.
//

class EmitterLoc :
	public IEmitterLoc									// Interface
	{
	public :
	EmitterLoc ( void );									// Constructor
...
//	adtValue		vLast;									// Last value processed
	...
	// 'IEmitterLoc' members
	STDMETHOD(connect)	( IReceptor *, bool );
	STDMETHOD(emit)		( const WCHAR *, const ADTVALUE & );
	};

//
// Class - EmitterIt.  Iterator for the emitter.
//
...
class EmitterIt :
	...
	{
	public :
	EmitterIt ( Emitter *, bool ); 					// Constructor

	// Run-time data
	Emitter	*pEmit;										// Parent emitter
	bool		bKeys;										// Iterating keys/values
	int		iIdx;											// Enumeration index
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(EmitterIt)
		CCL_INTF(IIt)
	CCL_OBJECT_END()
	virtual void		destruct		( void );		// Destruct object
...
//
// Class - EmittertIt.  Iterator for the temporal emitter.
//
...
class EmittertIt :
	...
	{
	public :
	EmittertIt ( Emittert *, bool ); 				// Constructor

	// Run-time data
	Emittert			*pEmit;								// Parent emitter
	bool				bKeys;								// Iterating keys/values
	NSSQNM			at;									// Iterator position
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(EmittertIt)
		CCL_INTF(IIt)
	CCL_OBJECT_END()
	virtual void		destruct		( void );		// Destruct object
...
//
// Class - Emittert.  Default temporal emitter class.
//
...
class Emittert :
	...
	{
	public :
	Emittert ( TemporalImpl *, U64 );				// Constructor

	// Run-time data
	TemporalImpl	*pTmp;								// Temporal database
//	IByteStream		*pStm;								// Emitter value stream
	U64				uId;									// Emitter Id
...
//
// Class - Graph.  Implementation of a graph manipulator.
//

class Graph :
	public CCLObject,										// Base class
	public IGraph,											// Interface
	public IDictionary									// Interface
	{
	public :
	...
	INamespace		*pSpc;								// Reference to namespace
	Receptor			*prDsc;								// Descriptor receptor
	IDictionary		*pDctDsc;							// Active descriptor
	IDictionary		*pDctCns;							// Connection descriptors
	IDictionary		*pDctItms;							// Item descriptors
	DWORD				dwItms,dwConn;						// Notification keys
...
//
// Class - GraphConnect.  Implementation of the graph connections dictionary.
//
...
class GraphConnect :
	...
	{
	public :
	GraphConnect ( void );								// Constructor

	// Run-time data
	IDictionary		*pDctc;								// Contained dictionary
	INamespace		*pSpc;								// Reference to namespace
	IDictionary		*pGr;									// Reference to owner graph
	bool				bGrRef;								// Reference graph
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(GraphConnect)
		CCL_INTF(IDictionary)
		CCL_INTF(IContainer)
	CCL_OBJECT_END()
	virtual HRESULT	construct( void );			// Construct object
	...
	HRESULT	connStore	( int, IDictionary * );	// Store connection

	};

//
// Class - GraphItems.  Implementation of the graph items dictionary.
//
...
class GraphItems :
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(GraphItems)
		CCL_INTF(IDictionary)
		CCL_INTF(IContainer)
	CCL_OBJECT_END()
	virtual HRESULT	construct( void );			// Construct object
	...
	// Internal utilities
	HRESULT nodeAdd ( const WCHAR *,					// Add node to graph
							IDictionary * );				
	HRESULT subAdd ( const WCHAR *,					// Add subgraph to graph
							IDictionary * );				
...
/*
//
// Class - Node.  Implementation of a node.
//
...
class Node :
	...
	// Run-time data
	IUnknown			*punkDct;							// Context
	IDictionary		*pDct;								// Context
	IDictionary		*pConnsAdd;							// Added connectors
	IBehaviour		*pBehave;							// Behaviour
	INamespace		*pSpc;								// Attached namespace
	bool				bReceive;							// Receive processing enabled.
	IReceptor		*prRx;								// Receiving receptor
	IReceptor		*prDsc;								// Descriptor
	IEmitter			*peDsc;								// Emitters
	const ADTVALUE	*pvRx;								// Receiving value
	sysCS				csRx,csInt;							// Thread safety
	IList				*pRecepQ;							// Receiver queue
	IIt				*pInQ;								// Input iterator
	adtString		strEmit;								// Emitter names
	adtLong			vR;									// Internal
	adtValue			vQ;									// Internal

	// 'INode' members
	STDMETHOD(connector)	( const WCHAR *, IUnknown **, bool );

	// 'IReceiver' members
	STDMETHOD(receive)	( IReceptor *, const ADTVALUE & );

	// CCL
	CCL_OBJECT_BEGIN(Node)
		CCL_INTF(INode)
		CCL_INTF(IReceiver)
		CCL_INTF_AGG(IDictionary,punkDct)
		CCL_INTF_AGG(IContainer,punkDct)
	...
	// Internal utilities
	HRESULT attach		( IBehaviour * );
	HRESULT process	( IReceptor *, const ADTVALUE & );
	HRESULT space		( void );
...
/*
//
// Class - Receptor.  Implementation of a receptor.
//
...
class Receptor :
	...
	{
	public :
	Receptor ( void );									// Constructor

	// Run-time data

	// 'IReceptor' members
	STDMETHOD(connected)	( IEmitter *, bool );
	STDMETHOD(receive)	( const ADTVALUE & );

	// CCL
	CCL_OBJECT_BEGIN(Receptor)
		CCL_INTF(IReceptor)
		CCL_INTF(IDictionary)
		CCL_INTF(IContainer)
	CCL_OBJECT_END()
...
//
// Class - ReceptorIt.  Iterator for the receptor.
//
...
class ReceptorIt :
	...
	{
	public :
	ReceptorIt ( Receptor *, bool ); 				// Constructor

	// Run-time data
	Receptor	*pRcp;										// Parent receptor
	bool		bKeys;										// Iterating keys/values
	bool		bFirst;										// First receptor ?
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(ReceptorIt)
		CCL_INTF(IIt)
	CCL_OBJECT_END()
	virtual void		destruct		( void );		// Destruct object
...
//
// Class - Receptors.  Enhanced dictionary for emitter receptor connections.
//
...
class Receptors :
	...
	{
	public :
	Receptors ( Emitter * );							// Constructor

	// Run-time data
	Emitter			*pEmt;								// Parent emitter
	IDictionary		*pDct;								// Contained dictionary
	...
	// CCL
	CCL_OBJECT_BEGIN_INT(Receptors)
		CCL_INTF(IDictionary)
		CCL_INTF(IContainer)
	CCL_OBJECT_END()
	virtual HRESULT	construct( void );			// Construct object
...
//
// Class - Receptort.  Internal class to receive emissions to be recorded.
//
...
class Receptort :
	...
	{
	public :
	Receptort ( IEmitter *, IDictionary *,			// Constructor
						bool = true );						

	// Run-time data
	IEmitter			*pEmit;								// Source emitter
	IDictionary		*pEmitt;								// Temporal access
	bool				bSetup;								// Setup phase ?

	// 'IReceptor' members
	STDMETHOD(connected)	( IEmitter *, bool );
	STDMETHOD(emitters)	( IContainer ** );
	STDMETHOD(receive)	( const ADTVALUE & );
	STDMETHOD(receiver)	( IReceiver *, const WCHAR * );
	STDMETHOD(receiver)	( IReceiver **, ADTVALUE * );

	// CCL
	CCL_OBJECT_BEGIN_INT(Receptort)
		CCL_INTF(IReceptor)
	CCL_OBJECT_END()
	virtual void		destruct		( void );		// Destruct object
